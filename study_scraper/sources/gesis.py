"""GESIS Knowledge Graph SPARQL source (lake-style, A14 / Q16-v2).

The GESIS Knowledge Graph at `data.gesis.org/gesiskg/sparql` is a
**public** SPARQL endpoint. No auth (per Q17 investigation
2026-05-29: the legacy connector's `GESIS_API_KEY` was dead config
and never read). This source ingests **catalog metadata** for
GESIS-hosted research data -- Politbarometer, ALLBUS, UBA
Umweltbewusstseinsstudie, GLES, ESS, EVS/WVS, and many more, all
under one SPARQL endpoint.

Microdata file downloads (the actual SPSS .sav / Stata .dta / CSV
bodies) live behind `search.gesis.org`'s post-login flow -- not in
this source. See Q19 (deferred).

This source emits one `SourceRecord` per `schema:Dataset` URI. The
payload preserves the SPARQL SELECT result rows for that dataset
(one entry per `(predicate, object)` triple), so we can later add a
view extracting `(title, abstract, creator, issued, variables, ...)`
as typed columns without re-fetching.

Modes (mirror DAWUM / SSOAR / OpenAlex):

  * `from_file=<path>` -- reads a saved JSON document from disk in
    a small wrapper shape (see fixture). Used by tests and by
    environments without outbound HTTP.
  * Live (default) -- runs SPARQL queries against the public
    endpoint.

Pagination: LIMIT/OFFSET on the catalog query, then per-dataset
detail fetch in batches.
"""
...
GESIS_LICENSE = "GESIS terms of use (public catalog)"
RECORD_FORMAT = "gesis_kg_sparql_json"

# Catalog of dataset URIs (offset-paginated).
_CATALOG_QUERY = """
...
# All (predicate, object) for a batch of dataset URIs.
# VALUES inside ?d so a single round-trip covers up to ~25 datasets.
_BATCH_DETAIL_QUERY = """
PREFIX schema: <https://schema.org/>
SELECT ?d ?p ?o WHERE {
  VALUES ?d { %s }
...
class GESISSource:
    """Lake source for the GESIS Knowledge Graph SPARQL endpoint."""
    ...
    def __enter__(self) -> "GESISSource":
        return self
    ...
    def iter_records(
        ...
        *,
        ...
        """Yield one SourceRecord per `schema:Dataset` URI.

        Live mode: paginate the catalog, batch-fetch details, emit.
        Fixture mode: read a saved JSON document, emit.
        """
        ...
        *,
        ...
        if SPARQLWrapper is None:
            raise RuntimeError(
                ...
                "environment to use the live GESIS source."
            )
        now = datetime.now(timezone.utc)
        offset = 0
        yielded = 0
    ...
    def _build_record(
        ...
        *,
        ...
        # Pull out title / license / doi as typed hints; everything
        # else goes verbatim into `payload.triples` for the lake.
        ...
        return SourceRecord.build(
            ...
            payload=payload,
            doi=doi,
            license=license_uri or GESIS_LICENSE,
            provenance={
                "fetch_source": "gesis_kg_sparql",
