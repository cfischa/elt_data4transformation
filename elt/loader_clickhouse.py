...
def _coerce_to_list(value: Any) -> List[Any]:
    """Helper to normalise values into a list."""
...
class ClickHouseConfig(BaseModel):
    ...
    database: str = Field(
        default_factory=lambda: os.getenv("CLICKHOUSE_DATABASE", "analytics")
    )
...
class ClickHouseLoader:
    ...
    def insert_dataframe(
        self, table_name: str, df: pd.DataFrame, batch_size: int = 10000
    ) -> int:
        """
        ...
        try:
            ...
            for i in range(0, total_rows, batch_size):
                ...
                self.logger.debug(
                    f"Inserted batch {i//batch_size + 1}: {len(data)} rows"
                )
    ...
    def insert_json_data(
        self, table_name: str, json_data: List[Dict[str, Any]], batch_size: int = 10000
    ) -> int:
        """
    ...
    def upsert_data(
        self, table_name: str, data: List[Dict[str, Any]], primary_key: str
    ) -> int:
        """
    ...
    def load_from_file(
        self, table_name: str, file_path: Union[str, Path], file_format: str = "auto"
    ) -> int:
        """
    ...
    def log_ingestion(self, ingestion: RawIngestion) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Logged ingestion: {ingestion.source}.{ingestion.dataset}"
            )
    ...
    def load_dicts(
        self, data: List[Dict[str, Any]], table: str, batch_size: int = 10000
    ) -> int:
        """
        ...
        try:
            ...
            for i in range(0, total_rows, batch_size):
                ...
                self.logger.debug(
                    f"Loaded batch {i//batch_size + 1}: {len(batch)} rows into {table}"
                )
    ...
    def upsert_dawum_polls(
        ...
        try:
            ...
            if poll_ids:
                ...
                self.logger.info(
                    f"Deleted existing records for {len(poll_ids)} poll_ids"
                )

            # Insert new data
            rows_loaded = self.load_dicts(
                data, "raw.dawum_polls", batch_size=batch_size
            )
    ...
    def upsert_gesis_metadata(
        ...
        try:
            ...
            total_loaded = self.load_dicts(
                processed_data, "raw.gesis_metadata", batch_size
            )

            self.logger.info(
                f"Successfully upserted {total_loaded} GESIS metadata records"
            )
            return total_loaded
    ...
    def create_eurostat_metadata_table(self) -> None:
        """Create the raw.eurostat_metadata table."""
        ...
        try:
            self.client.command("CREATE DATABASE IF NOT EXISTS raw")
            self.client.command(ddl)
            self.logger.info("Created raw.eurostat_metadata table")
        except Exception as exc:  # pragma: no cover - defensive
            self.logger.error("Failed to create raw.eurostat_metadata table: %s", exc)
            raise

    def upsert_eurostat_metadata(
        self,
        records: List[Dict[str, Any]],
        batch_size: int = 1000,
    ) -> int:
        """Upsert Eurostat metadata rows into ClickHouse."""

        if not records:
            self.logger.warning("No Eurostat metadata to upsert")
            return 0

        self.create_eurostat_metadata_table()
        ...
        for item in records:
            dataset_code = str(
                ...
                or item.get("code")
                or item.get("id")
                or (item.get("dataset") or {}).get("code")
                or ""
            )
            if not dataset_code:
                self.logger.debug(
                    "Skipping Eurostat metadata row without dataset code: %s", item
                )
                continue

            title = str(item.get("title") or dataset_code)
            description = str(item.get("description") or title)

            last_update = item.get("last_update") or item.get("lastUpdate")
            if isinstance(last_update, str) and last_update:
                cleaned = last_update.replace("Z", "+00:00")
                try:
            ...
            if not isinstance(keywords, list):
                keywords = _coerce_to_list(keywords)

            themes = item.get("themes") or item.get("categories") or []
            if not isinstance(themes, list):
                themes = _coerce_to_list(themes)

            dimensions = item.get("dimensions") or item.get("dimension_ids") or []
            if not isinstance(dimensions, list):
                dimensions = _coerce_to_list(dimensions)

            raw_metadata = item.get("raw") or item.get("raw_metadata") or item
            if isinstance(raw_metadata, str):
                raw_text = raw_metadata
            else:
                try:
                    raw_text = json.dumps(raw_metadata, ensure_ascii=False)
                except (TypeError, ValueError):
                    raw_text = json.dumps({}, ensure_ascii=False)
            ...
            processed.append(
                {
                    ...
                    "keywords": [str(v) for v in keywords if v],
                    "themes": [str(v) for v in themes if v],
                    "dimensions": [str(v) for v in dimensions if v],
                    "values_count": int(
                        item.get("values_count") or item.get("valuesCount") or 0
                    ),
        ...
        if not processed:
            self.logger.warning(
                "No Eurostat metadata rows to load after preprocessing"
            )
            return 0

        return self.load_dicts(processed, "raw.eurostat_metadata", batch_size)

    def create_soep_metadata_table(self) -> None:
        """Create the ``raw.soep_metadata`` table if it does not exist."""

        ddl = """
        CREATE TABLE IF NOT EXISTS raw.soep_metadata (
            slug String,
            title String,
            subtitle String,
            description String,
        ...
        try:
            self.client.command("CREATE DATABASE IF NOT EXISTS raw")
            self.client.command(ddl)
            self.logger.info("Created raw.soep_metadata table")
        except Exception as exc:
            self.logger.error(f"Failed to create raw.soep_metadata table: {exc}")
            raise

    def upsert_soep_metadata(
        self, data: List[Dict[str, Any]], batch_size: int = 100
    ) -> int:
        """Upsert SOEP indicator metadata into ``raw.soep_metadata``."""

        if not data:
            self.logger.warning("No SOEP metadata to upsert")
            return 0

        try:
            self.create_soep_metadata_table()
            ...
            for item in data:
                ...
                available_years = item.get("available_years") or []
                if isinstance(available_years, list):
                    years = []
                ...
                processed.append(
                    {
                        ...
                        "description": str(
                            item.get("long_description_html")
                        ...
                        "raw_summary": json.dumps(
                            item.get("raw_summary", {}), ensure_ascii=False
                        ),
                        "raw_detail": json.dumps(
                            item.get("raw_detail", {}), ensure_ascii=False
                        ),
                    }
                )

            inserted = self.load_dicts(processed, "raw.soep_metadata", batch_size)
            self.logger.info(
                "Successfully upserted %s SOEP metadata records", inserted
            )
            return inserted

        except Exception as exc:
            self.logger.error(f"Failed to upsert SOEP metadata: {exc}")
            raise
